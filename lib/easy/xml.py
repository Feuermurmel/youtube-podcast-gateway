...
class Node:
    ...
    def walk(self, fn=None, *, name=None, attrs=None, prune=True, root=False):
        """
        Iterate over all the elements inside this node which mach the given
        specification.

        :param name:
            If specified. Only return element with the given name.
        :param attrs:
            A map of attribute names to values. Only return elements which have
            all the specified attributes with the given values. Or which have
            the specified attributes with any value, if the value is given as
            None.
        :param prune:
            Do not return any child elements of elements that are also
            returned.
        :param root:
            Allow this element to be considered a match. If set to False, only
            child elements will be considered.
        """
        def find(elem):
    ...
    def find(self, *args, **kwargs):
        """
        Same as walk(), but returns exactly one element. Throws an exception if
        no or multiple elements match the specification.
        """
    ...
    def text(self):
        """
        Concatenation of the string contents of all TextNodes inside this
        element.
        """
...
class TextNode(Node):
    """
    Represents a text element between two XML tags.
    """
...
class ElementNode(Node):
    ...
    def __str__(self):
        attrs = ''.join(
            ' %s="%s"' % (k, v.translate(_xml_entities_encode_map))
...
class ParseHandler:
    """
    Internal class used by parse() to consume all parsed elements and build an
    XML tree.
    """
    def __init__(self):
        self._element_stack = []
...
def parse(str):
    ...
    pattern = alt(grp(tag_anon, 'tag'), grp(tagend_anon, 'tagend'), grp(data, 'data'), declaration)

    for i in matchall(pattern, str):
